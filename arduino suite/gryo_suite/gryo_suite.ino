...
double PitchGyroValDouble; // running sums get very large. We need to poll the int from the register and convert it to a double
double PitchGyroValRunSum = 0; // when turning, we poll the gyro thousands of times and accumulate the values help in yawGyroVal
double RollGyroValDouble; // running sums get very large. We need to poll the int from the register and convert it to a double
double RollGyroValRunSum = 0; // when turning, we poll the gyro thousands of times and accumulate the values help in yawGyroVal
...
void loop(){
   ...
   {
      gyroTurnTimeoutError = gyroBasedTurns(CLOCKWISE, 1.0);
...
void printVals()
{
  ...
  pitchGyroD = pitchGyroVal;
  
  if(abs(pitchGyroD) > abs(gyroNoiseThresh)){
    totalClicks += pitchGyroD;
  }
  }
